...
ISR(TIMER1_OVF_vect)
{
    ...
    if (value != '\0') {  // Data available from UART
        ...
        uart_puts("\x1b[4;32m");  // 4: underline style; 32: green foreground
        uart_putc(value); //receiver character
        uart_puts("\t");
        uart_puts("\x1b[0m");     // 0: reset all attributes
        itoa(value, string, 10); // decimal nuber of character
        uart_puts(string);
        uart_puts("\t");
        itoa(value, string, 16); // hexadecimal number of character
        uart_puts("0x");
        uart_puts(string);
        uart_puts("\t");
        itoa(value, string, 2); // binary number of character 
        uart_puts("0b");
        uart_puts(string);
        uart_puts("\r\n");
        /*
